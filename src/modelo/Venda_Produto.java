...
@NamedQueries({
    @NamedQuery(name = "venda_produto.ExistProduct", query = "SELECT vp FROM Venda_Produto vp WHERE vp.produto.idProduto = :id_produto AND vp.venda.idVenda = :id_venda"),
    @NamedQuery(name = "venda_produto.findByVenda", query = "SELECT vp FROM Venda_Produto vp WHERE vp.venda.idVenda = :id_venda")
})
public class Venda_Produto implements Serializable{
    ...
    public int hashCode() {
        int hash = 5;
        hash = 59 * hash + Objects.hashCode(this.venda_ProdutoPK);
        return hash;
    }
